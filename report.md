...
> **Note:** This investigation uses a historical Zeek DNS dataset captured in March 2012. Some NXDOMAIN results are expected due to services that have since been retired or renamed.
...
**Finding:** Host `10.10.117.210` generated significantly more DNS traffic than any other system.
...
**Verdict:** Benign — the system is consistent with a DNS resolver or gateway rather than malicious activity.
...
**Finding:** 49,923 total NXDOMAIN events across 87 unique hosts. Top host: `192.168.202.103` (7,409 failed lookups).
...
**Verdict:** Benign — these failures are consistent with the age of the dataset rather than DGA activity.
...
**Finding:** 2,460 host/domain pairs displayed regular hourly query intervals.
...
**Finding:** One host generated repeated long, high-entropy DNS queries.

- **Source Host:** `192.168.204.71`
- **Destination:** `auth.rssfeeds.com`, `connect.rssfeeds.com`
...
Another host appeared during wildcard searching: `192.168.202.106`. Further investigation confirmed it was communicating with `rssfeeds.usatoday.com` using normal A record lookups. This was ruled out as a false positive.

**Verdict:**  **Malicious** — evidence is consistent with DNS tunneling and DNS-based command-and-control communication.
...
1. Isolate `192.168.204.71`
2. Block `rssfeeds.com` and all subdomains
3. Acquire memory and disk images for forensic analysis
4. Review authentication and endpoint logs for lateral movement
5. Escalate to Incident Response
6. Reimage the compromised host after forensic acquisition
